...
class AIServiceProvider:
    ...
    def __init__(self):
        ...
        self.hf_router_base_url = "https://router.huggingface.co/hf-inference"
...
""".strip()
    ...
    async def text_to_speech_hf(self, text: str) -> Dict:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                # Try the new HF router endpoint first
                url = f"{self.hf_router_base_url}/{self.hf_tts_model}"
                async with session.post(
                    url,
                    headers=headers,
                    json=payload
                    ...
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        return {
                            "error": f"HF TTS API error: {error_text}",
                            "success": False
                        }
                        
        except Exception as e:
            return {
                "error": f"HF TTS conversion failed: {str(e)}",
                "success": False
            }
    ...
    async def text_to_speech_groq(self, text: str, voice: Optional[str] = None, response_format: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            print(f"[DEBUG] Groq TTS request: model={payload['model']}, voice={payload['voice']}, format={fmt}")
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    print(f"[DEBUG] Groq TTS response status: {response.status}")
                    if response.status == 200:
                        audio_bytes = await response.read()
                        import base64
                        audio_b64 = base64.b64encode(audio_bytes).decode()
                        ...
                        error_msg = f"Groq TTS API error ({response.status}): {msg}"
