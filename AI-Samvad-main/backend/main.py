...
async def text_to_speech(text: str = Form(...), voice: Optional[str] = Form(None), response_format: Optional[str] = Form(None)):
    ...
    print(f"[TTS] Received text: {text[:50]}..., voice: {voice}, response_format: {response_format}")
    ...
    print(f"[TTS] Groq result: success={result.get('success')}, error={result.get('error', 'N/A')}")
    
    if result.get("success"):
        return result
    
    # Fallback to HF TTS if Groq fails
    print(f"[TTS] Trying Hugging Face fallback...")
    try:
        hf = await ai_service.text_to_speech_hf(text)  # type: ignore
        print(f"[TTS] HF result: success={hf.get('success')}, error={hf.get('error', 'N/A')}")
        if hf.get("success"):
            hf["note"] = f"Groq TTS failed; fallback used. Groq error: {result.get('error')}"
            return hf
    except Exception as e:
        # ignore and raise below
        print(f"[TTS] HF fallback error: {e}")
        result["fallback_error"] = str(e)
    
    print(f"[TTS] Raising error: {result.get('error', 'TTS failed')}")
